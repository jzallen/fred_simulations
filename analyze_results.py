    ...
    def create_disease_plot(ax, disease_data):
        ...
        if len(numeric_cols) > 0:
            for i, col in enumerate(numeric_cols[:3]):  # Plot up to 3 metrics
                if i < 3:
                    ax.plot(disease_data['Day'] if 'Day' in disease_data.columns else range(len(disease_data)), 
                                   disease_data[col], marker='o', label=col)
            ...
            ax.text(0.5, 0.5, 'No numeric disease data\navailable for plotting', 
