...
class TestStopSession:
    ...
    def test_stop_session__when_single_session_found__sends_sigint(self, mock_run):
        ...
        assert mock_run.call_args_list[0] == call(
            'pgrep -a "tcr:*" | grep "tcr:my-feature"', shell=True, capture_output=True, text=True
        )
    ...
    def test_stop_session__when_no_session_found__logs_error(self, mock_run):
        ...
        mock_run.assert_called_once_with(
            'pgrep -a "tcr:*" | grep "tcr:nonexistent"', shell=True, capture_output=True, text=True
        )
    ...
    def test_stop_session__when_multiple_sessions_found__stops_all(self, mock_run):
        ...
        assert any(
            "Multiple sessions found matching 'test': PIDs 12345, 67890" in str(call)
            for call in calls
        )
    ...
    def test_stop_session__when_pgrep_not_found__logs_error(self, mock_run):
        ...
        mock_logger.error.assert_called_with(
            "pgrep or kill command not found. Please ensure procps is installed."
        )
