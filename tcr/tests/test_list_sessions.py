...
class TestListSessions:
    ...
    def test_list_sessions__when_no_sessions_running__logs_no_sessions(self, mock_run):
        ...
        mock_run.assert_called_once_with(["pgrep", "-a", "tcr:"], capture_output=True, text=True)
        mock_logger.info.assert_called_once_with("No TCR sessions currently running")

    @patch("subprocess.run")
    @patch("builtins.print")
    def test_list_sessions__when_single_session_running__prints_raw_output(
        self, mock_print, mock_run
    ):
        """Test that list_sessions prints raw pgrep output for a single session."""
        # pgrep -a output format: PID command
        mock_run.return_value = Mock(returncode=0, stdout="12345 tcr:my-feature\n")
    ...
    @patch("subprocess.run")
    @patch("builtins.print")
    def test_list_sessions__when_multiple_sessions_running__prints_all_sessions(
        self, mock_print, mock_run
    ):
    ...
    def test_list_sessions__when_pgrep_not_found__logs_error(self, mock_run):
        ...
        mock_logger.error.assert_called_with(
            "pgrep command not found. Please ensure procps is installed."
        )
