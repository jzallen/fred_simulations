...
from epistemix_platform.repositories import SQLAlchemyRunRepository
...
class TestSQLAlchemyJobRepository:
    ...
    def test_save__given_valid_new_run__run_persisted_on_commit(
        self, repository: IRunRepository, db_session
    ):
    ...
    def test_save__given_existing_run__updates_run_on_commit(
        self, repository: IRunRepository, db_session
    ):
        run = Run.create_unpersisted(
            job_id=1,
    ...
    def test_delete__given_existing_run__deletes_run_on_commit(
        self, repository: IRunRepository, db_session
    ):
        run = Run.create_unpersisted(
            job_id=1,
    ...
    def test_find_by_job_id__given_existing_job_id__returns_runs(
        self, repository: IRunRepository, db_session
    ):
    ...
    def test_find_by_job_id__given_non_existing_job_id__returns_empty_list(
        self, repository: IRunRepository
    ):
        runs = repository.find_by_job_id(999)
        assert runs == []

    def test_find_by_user_id__given_existing_user_id__returns_runs(
        self, repository: IRunRepository, db_session
    ):
        run1 = Run.create_unpersisted(
            job_id=1,
    ...
    def test_find_by_user_id__given_non_existing_user_id__returns_empty_list(
        self, repository: IRunRepository
    ):
        runs = repository.find_by_user_id(999)
        assert runs == []

    def test_find_by_status__given_existing_status__returns_runs(
        self, repository: IRunRepository, db_session
    ):
        run1 = Run.create_unpersisted(
            job_id=1,
    ...
    def test_find_by_status__given_non_existing_status__returns_empty_list(
        self, repository: IRunRepository
    ):
