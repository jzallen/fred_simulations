...
class TestGetJobUploads:
    ...
    def test_get_job_uploads__job_not_found__raises_value_error(self, job_repository, run_repository):
    ...
    def test_get_job_uploads__job_with_no_uploads__returns_empty_list(self, job_repository, run_repository):
    ...
    def test_get_job_uploads__job_with_input_location__returns_job_input_upload(self, job_repository, run_repository):
        ...
        upload = uploads[0]
        assert upload.upload_type == "job_input"
        assert upload.job_id == 1
        assert upload.location.url == "https://s3.amazonaws.com/bucket/job_1_input"
        assert upload.run_id is None
    
    def test_get_job_uploads__job_with_config_location__returns_job_config_upload(self, job_repository, run_repository):
        ...
        assert len(uploads) == 1
        upload = uploads[0]
        assert upload.upload_type == "job_config"
        assert upload.job_id == 1
        assert upload.location.url == "https://s3.amazonaws.com/bucket/job_1_config"
        assert upload.run_id is None
    
    def test_get_job_uploads__job_with_runs__returns_run_uploads(self, job_repository, run_repository):
    ...
    def test_get_job_uploads__complete_job__returns_all_uploads(self, job_repository, run_repository):
        ...
        assert "job_input" in upload_types
        assert "job_config" in upload_types
        assert "run_output" in upload_types
