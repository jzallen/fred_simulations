...
from epistemix_api.models.upload_content import UploadContent, ContentType
...
def write_to_local(file_path: Path, content: UploadContent, force: bool = False) -> None:
    ...
    if content.content_type == ContentType.ZIP_ARCHIVE:
        ...
        try:
            file_path.write_text(content.raw_content, encoding=content.encoding)
            logger.info(f"Wrote {content.content_type.value} content to {file_path} ({content.get_size()} bytes)")
        except Exception as e:
            raise IOError(f"Failed to write {content.content_type.value} content to {file_path}: {e}")